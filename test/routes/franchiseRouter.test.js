...
test("create franchise", async () => {
  ...
  let createFranchiseRes = await request(app)
    .post("/api/franchise")
...
test("get franchises", async () => {
  let getFranchisesRes = await request(app).get(
    `/api/franchise?page=0&limit=10&name=${newFranchise.name}`,
  );
  expect(getFranchisesRes.status).toBe(200);
  expect(getFranchisesRes.body.franchises).toEqual(
    expect.arrayContaining([
      expect.objectContaining({ name: newFranchise.name }),
    ]),
  );
});
...
test("create store", async () => {
  ...
  let createStoreRes = await request(app)
  ...
  createStoreRes = await request(app)
    .post(`/api/franchise/${newFranchiseId}/store`)
    .set("Authorization", `Bearer ${adminUserAuthToken}`)
    .send(newStore);
...
test("get user franchises", async () => {
  ...
  let userFranchisesRes = await request(app)
  ...
  userFranchisesRes = await request(app)
    .get(`/api/franchise/${adminUserId}`)
    .set("Authorization", `Bearer ${adminUserAuthToken}`);
  expect(userFranchisesRes.status).toBe(200);
...
test("delete store", async () => {
  ...
  let deleteStoreRes = await request(app)
    .delete(`/api/franchise/${newFranchiseId}/store/${newStoreId}`)
    .set("Authorization", `Bearer ${testUserAuthToken}`);
